...
def rho_psigma_pse(rootfile) : 
  ...
  titles = ['PS E and Rho P#Sigma status', 'Photon beam energy from PS pair E (GeV) [diamond]', 'PS E err (diamond)', 'Photon beam energy from PS pair E (GeV) [amo peak]', 'PS E err (amo)', 'Abs(P#Sigma) from #rho(770) production', 'Abs(P#Sigma)_err', 'P#Sigma (0)', 'P#Sigma(0)err', 'P#Sigma (90)', 'P#Sigma(90)err', 'P#Sigma (135)', 'P#Sigma(135)err', 'P#Sigma (45)', 'P#Sigma(45)err','#phi_{0} diamond', '#phi_{0} diamond err', '#phi_{0} amo', '#phi_{0} amo err' ]   # Graph titles
...
#  dirname = '/PS_flux/PSC_PS'      # directory containing the histogram
  dirname = '/PSPair/PSC_PS'      # directory containing the histogram
...
#    hps = get_histo(rootfile, dirname, histoname, min_counts)
  ...
  if amo :     # for amo, just take fitted peak,  # for diamond, find steepest part of edge
    ...
    g = TF1('g','gaus',psmaxe-0.05, psmaxe+0.05) 
